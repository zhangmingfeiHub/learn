...
public class Recv11 {
	...
	public static void main(String[] args) throws IOException, TimeoutException {
		 ...
		 * queue: 队列名
		 * durable：是否持久化
		 * exclusive：是否排他，true表示私有，只有当前消费者才能消费该队列消息，其他消费者尝试连接该队列会报异常
		...
		channel.basicConsume(QUEUE_NAME_2, false, new DefaultConsumer(channel) {
			...
			public void handleDelivery(String consumerTag, Envelope envelope, BasicProperties properties, byte[] body)
				...
				try {
					...
					System.err.println(" [x-1] recieve msg: " + msg + ", deliveryTag: " + envelope.getDeliveryTag());
					
					channel.basicAck(envelope.getDeliveryTag(), false);
				} catch (Exception e) {
					System.err.println(" [x-1] recieve msg exception: " + e + ", deliveryTag: " + envelope.getDeliveryTag());
